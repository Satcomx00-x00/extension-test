document.addEventListener('DOMContentLoaded', function() {
  const chatgptUrlInput = document.getElementById('chatgpt-url') as HTMLInputElement;
  const saveSettingsButton = document.getElementById('save-settings') as HTMLButtonElement;
  const autoAnswerButton = document.getElementById('auto-answer') as HTMLButtonElement;
  const statusMessage = document.getElementById('status-message') as HTMLDivElement;

  // Load saved settings
  chrome.storage.local.get(['chatgptUrl'], function(data: {chatgptUrl?: string}) {
    if (data.chatgptUrl) {
