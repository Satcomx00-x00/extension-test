...
async function processMicrosoftForm(): Promise<boolean> {
  try {
    ...
    // Convert NodeListOf<Element> to Array<Element> to fix the iterator issue
...
async function processMicrosoftFormQuestion(questionElement: Element): Promise<void> {
  try {
    ...
    if (questionType === 'short-text' || questionType === 'paragraph') {
      ...
      let bestMatch = microsoftFormsFindBestMatchingOption(options, answer);
      ...
      const optionsToSelect = microsoftFormsFindCheckboxOptionsToSelect(options, answer);
      
      for (const option of optionsToSelect) {
        const checkboxInput = option.querySelector('input[type="checkbox"]');
        if (checkboxInput) {
          (checkboxInput as HTMLInputElement).click();
        }
      }
    }
...
// Helper functions specific to Microsoft Forms
function microsoftFormsFindBestMatchingOption(options: Element[], answer: string): Element | null {
  try {
...
function microsoftFormsFindCheckboxOptionsToSelect(options: Element[], answer: string): Element[] {
  try {
    const answerLower = answer.toLowerCase();
    ...
    if (optionsToSelect.length === 0) {
      ...
      if (hasMultipleIndicator) {
        ...
        const bestMatch = microsoftFormsFindBestMatchingOption(options, answer);
